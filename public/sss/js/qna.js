...
function answer(id) {
	  if ($('#'+id).find('.answer_block'+id).html()) { $('.answer_block'+id).remove()}
		...
		var fhN = "回复："
	  	...
	  	<div class="Csunbmit"><label class="Cnick"></label><input type="hidden" class="Cusername" ></input>\
					<button class="submitBtn"  class="Cusername" onclick=submit_pl("'+cname+'")>确定回复</button></div></div>';
	   $('#'+id).append(fhHtml);
...
function like(id) {
	...
	likeID = id
	ajaxPostReq('/pingluns/php/like.php',{'id':id,'like':likes,'time':get_time()},{"1":'来啊 来啊 来一遍啊!!!  么么哒！','0':"亲~您已经赞过了哦!! "},voids)
}
...
function submit_pl(cname) {
	...
	data.time = time
	ajaxPostReq('/pingluns/php/add.php',data,{'0':'飞吻已发送','1':'起飞成功'},reflash)
}
