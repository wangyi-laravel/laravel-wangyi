...
function recursived($meta,$flag )
{
	...
	if ($child) {
		...
		$mid='';
		if ($flag==1) {
			$t = '<label style="float:right"><span class="bottomSpan" onclick="answer('.$id.')">回复</span >&nbsp;&nbsp;<span class="bottomSpan" onclick="like('.$id.')" id="support'.$id.'">点赞</span>(<span class="scount" id="like'.$id.'">'.$support.'</span>)</label>';
			$end = $content.$t.'</div>';
		    return $head.$mid.$end;
		}
